"""Command-line entry point: `kindle-weather render` and `kindle-weather deploy`."""
...
from kindle_weather.deploy import (
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="kindle-weather", description="Weather dashboard for jailbroken Kindle e-readers."
    )
...
def deploy(config: Config, image: Path, extension: Path, wait_seconds: int) -> None:
    if not image.is_file():
        raise DeployError(f"{image} not found, run `kindle-weather render` first")
    with tempfile.TemporaryDirectory() as workdir:
        ...
        if config.kindle.method == "usb":
            ...
            print("Copied to the Kindle. Eject it, then open KUAL > KindleWeather to display it.")
        else:
